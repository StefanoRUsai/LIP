...
let rec evalInt e r envType pila= match (sem e r envType pila ) with
  Int n -> n
| _ -> raise TypeMismatch
 
and evalBool e r envType pila= match sem e r envType pila with
  Bool b -> b
| _ -> raise TypeMismatch

and evalChar e r envType pila= match sem e r envType pila with
  Char c -> c
| _ -> raise TypeMismatch
...
and  sem e r envType pila = match e with
   Eint n -> Int n
  | Echar c -> Char c  
  | Val x ->  applyenv (r,x)
  ...
  | True  -> Bool true
  | False -> Bool false
  | Eq (e1,e2) -> (match sem e1 r envType pila, sem e2 r envType pila with
  ...
  | Empty -> List []
  | Head e1 -> let a = sem e1 r envType pila in 
               let b = (match a with
                        List [] -> failwith "Lista vuota" 
                      | List (hd::tl) -> hd  
                      | _ -> raise TypeMismatch)         
               in b 
  |Tail e1 -> let a = sem e1 r envType pila in 
              let b = (match a with
                       List [] -> failwith "Lista vuota" 
                     | List (hd::tl) -> tl  
                     | _ -> raise TypeMismatch)         
    ...
    | Cons (e1, e2) -> (if (not (typeinf_App (Cons (e1, e2)) envType = TInt)) then
        (let a = sem e2 r envType pila in 
                                let b = (match a with                     
                                           |(List l) ->(match l with
                                                            [] -> (sem e1 r envType pila)::[]
                                                          |(hd::tl) as lista -> (sem e1 r envType pila)::lista)  
                                           |_->failwith "errore liste match typeinf")
                                in  List b ) else failwith "liste")
  |Epair (e1,e2) -> Pair ( sem e1 r envType pila, sem e2 r envType pila) 
  |Fst e -> ( match (sem e r envType pila) with
           Pair (a, b) -> a
              |_-> raise TypeMismatch)
  |Snd e -> ( match (sem e r envType pila) with
           Pair (a, b) -> b
          |_-> raise TypeMismatch) 
  | Ifthenelse(e0,e1,e2) -> if evalBool e0 r envType pila then sem e1 r envType pila else sem e2 r envType  pila
  | Let (x,e1,e2) -> sem e2 (bind (r,x,(sem e1 r envType pila)))
