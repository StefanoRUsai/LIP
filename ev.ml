...
((let x = 'c' in x)::[])::[]::[];;
sem (Cons ((Cons (Eint 3, Empty)),(Cons (Empty, Empty)))) emptyenv;;
[2;3]::[]::[];;
 sem  (Cons(Cons(Let(Ide "x", Echar 'c', Val (Ide "x")), Empty), Cons(Empty, Empty))) emptyenv;;
sem  (Let(Ide "x", Fun(Ide "y", Sum(Val(Ide "y"), Eint 3)), Appl(Val (Ide "x"), Eint 2))) emptyenv;;


typeinf (Cons((Cons (Eint 3, Empty)),(Cons (Empty, Empty))));;



sem   (Let(Ide "x", Echar 'c', Ifthenelse(Eq(Val(Ide "x"), Echar 'c'), Echar 'd', Echar 'f'))) emptyenv;;



sem   (Let(Ide "x", Eint 2, Sum(Val(Ide "x"),Eint 3)))  emptyenv;;
sem   (Let(Ide "x", True, And(Val(Ide "x"), False)))  emptyenv;;
sem   (Let(Ide "x", Echar 'c', Ifthenelse(Eq(Val(Ide "x"), Echar 'c'), Echar 'd', Echar 'f')) )emptyenv;;
sem   (Let(Ide "x", Fun(Ide "y", Sum(Val(Ide "y"), Eint 3)), Appl(Val (Ide "x"), Eint 2))) emptyenv;;
sem   (Appl(Fun(Ide "x", Val (Ide "x")), Eint 2)) emptyenv;;
sem   (Appl(Fun(Ide "x", Val (Ide "x")), True)) emptyenv;;
sem   (Appl(Fun(Ide "x", Val (Ide "x")), Echar 'c')) emptyenv;;

typeinf (Let(Ide "x", Echar 'c', Ifthenelse(Eq(Val(Ide "x"), Echar 'c'), Echar 'd', Echar 'f')) );;


sem   (Eq(Ifthenelse(True, Echar 'c', Echar 'd'), Ifthenelse(False, Eint 3, Eint 4))) emptyenv ;;


sem ( Eq(Epair(Eint 2, Echar 'c'),Epair(Echar 'd', Eint 3))) emptyenv ;;
sem ( Eq(Cons(Eint 2, Empty), Cons(Echar 'c', Empty))) emptyenv ;;
sem ( Eq(Ifthenelse(True, Echar 'c', Echar 'd'), Ifthenelse(False, Eint 3, Eint 4))) emptyenv ;;
sem ( Eq(Let(Ide "x", Eint 3, Sum(Val(Ide "x"), Eint 3)), Echar 'c')) emptyenv ;;
sem ( Cons(Cons(Fun(Ide "x", And(Val(Ide "x"), True)), Empty), Cons(Cons(Fun(Ide "x", Sum(Val(Ide"x"), Eint 3)), Empty), Empty))) emptyenv ;;



sem (Cons((Cons(Echar 'x',Empty)),(Cons(Empty,( Cons((Cons (Eint 3,Empty)),(Cons (Empty,(Cons (Empty, Empty)))))))))) emptyenv ;;
sem (Cons((Cons(Eint 3,Empty)),(Cons(Empty,( Cons((Cons (Eint 3,Empty)),(Cons (Empty,(Cons (Empty, Empty)))))))))) emptyenv ;;
