...
let rec applyenv (e:(ide*etype)list) (Ide i) =  match e,i with
    [],_ -> failwith "ambiente vuoto" 
  |  ((Ide a),(b:etype))::tl,i -> if a = i then b else applyenv tl (Ide i);;
  
(*associa un tipo e un identificatore all'ambiente dei tipi*)
let bind (e:(ide*etype)list) (i:ide) (t:etype) =(i,t)::e;; 
...
(***** è sbagliata sul sito del prof****)
let doubleboolnat = 
    Let(Ide "double",
	Fun(Ide "f", 
	    Fun (Ide "x", Apply(Val(Ide "f"), Apply(Val(Ide "f"),Val(Ide "x"))))),

	Let(Ide "a", Apply(Apply(Val(Ide "double"),Fun(Ide "x",
                              Apply(succ,Apply(succ,Val(Ide "x"))))),N 2),
